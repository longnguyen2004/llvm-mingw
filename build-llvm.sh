...
if [ -n "$HOST" ]; then
    ...
    if [ -n "$WITH_PYTHON" ]; then
        ...
        CMAKEFLAGS="$CMAKEFLAGS -DPYTHON_INCLUDE_DIRS=$PREFIX/python/include/python$PYTHON_VER"
        CMAKEFLAGS="$CMAKEFLAGS -DPYTHON_LIBRARIES=$PREFIX/python/lib/python$PYTHON_VER/config-$PYTHON_VER/libpython$PYTHON_VER.dll.a"
    fi
fi
