...
if [ -n "$HOST" ]; then
    ...
    if [ -n "$WITH_PYTHON" ]; then
        ...
        CMAKEFLAGS="$CMAKEFLAGS -DPYTHON_HOME=$PREFIX"
        CMAKEFLAGS="$CMAKEFLAGS -DLLDB_PYTHON_HOME=.."
        CMAKEFLAGS="$CMAKEFLAGS -DLLDB_PYTHON_RELATIVE_PATH=lib/python$PYTHON_VER/site-packages"
        
        CMAKEFLAGS="$CMAKEFLAGS -DPYTHON_EXECUTABLE=$PYTHON_EXEC"
        CMAKEFLAGS="$CMAKEFLAGS -DPYTHON_INCLUDE_DIRS=$PREFIX/include/python$PYTHON_VER"
        CMAKEFLAGS="$CMAKEFLAGS -DPYTHON_LIBRARIES=$PREFIX/lib/python$PYTHON_VER/config-$PYTHON_VER/libpython$PYTHON_VER.dll.a"
    fi
fi
...
cmake \
    ...
    -DLLVM_TOOLCHAIN_TOOLS="llvm-ar;llvm-ranlib;llvm-objdump;llvm-rc;llvm-cvtres;llvm-nm;llvm-strings;llvm-readobj;llvm-dlltool;llvm-pdbutil;llvm-objcopy;llvm-strip;llvm-cov;llvm-profdata;llvm-addr2line;llvm-symbolizer;llvm-readelf" \
